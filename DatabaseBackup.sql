...
BEGIN
  ...
  BEGIN
    ...
    AND NOT (@CurrentAvailabilityGroup IS NOT NULL AND @CurrentBackupType IN('FULL','SNAPSHOT') AND @CopyOnly = 'N' AND (@CurrentAvailabilityGroupRole <> 'PRIMARY' OR @CurrentAvailabilityGroupRole IS NULL))
    AND NOT (@CurrentAvailabilityGroup IS NOT NULL AND @CurrentBackupType IN('FULL','SNAPSHOT') AND @CopyOnly = 'Y' AND (@CurrentIsPreferredBackupReplica <> 1 OR @CurrentIsPreferredBackupReplica IS NULL) AND @OverrideBackupPreference = 'N')
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          -- Suspend database for snapshot backup
          ...
          -- Build payload with metadata tags (simplified to match working test)
